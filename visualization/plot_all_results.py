    ...
    for csv_file in csv_files:
        try:
            ...
            if not df.empty:
                ...
                # 解析文件名格式
                # 支持格式:
                # - ALT_full_q8_altzo10_optmudamw_lr1e-3_bs2
                # - FO_full_optmudamw_lr1e-3_bs2
                # - ZO_full_q8_optmudamw_lr1e-3_bs2
                mode = parts[0]
                scope = parts[1]
                ...
                for i, part in enumerate(parts):
                    ...
                    elif part.startswith('q') and part != 'q':
                        q = part.replace('q', '')
                    elif part.startswith('lr'):
                        lr = part.replace('lr', '')
                    elif part.startswith('altzo'):
                        alt_zo_steps = int(part.replace('altzo', ''))
                
                # 如果没有找到 batch_size，尝试从 CSV 数据中获取
                if batch_size is None and 'batch_size' in df.columns:
                    batch_size = df['batch_size'].iloc[0]
                elif batch_size is None:
                    batch_size = 2  # 默认值
                
                # 如果没有找到 lr，尝试从 CSV 数据中获取
                if lr is None and 'lr' in df.columns:
                    lr = df['lr'].iloc[0]
                elif lr is None:
                    lr = '1e-3'  # 默认值
                ...
                df['q'] = q
                df['lr'] = lr
                if alt_zo_steps is not None:
                    df['alt_zo_steps'] = alt_zo_steps
                # 为 ALT 模式创建更友好的实验名称
                if mode == 'ALT' and alt_zo_steps is not None:
                    df['experiment'] = f"{mode}_altzo{alt_zo_steps}"
                else:
...
def plot_loss_curves(df, output_dir="plots", figsize=(15, 10)):
    ...
    for exp in df['experiment'].unique():
        ...
        ax1.plot(exp_data['step'].values, exp_data['loss'].values, 
    ...
    for mode in df['mode'].unique():
        ...
        for exp in mode_data['experiment'].unique():
            ...
            ax2.plot(exp_data['step'].values, exp_data['loss'].values, 
    ...
    for scope in df['scope'].unique():
        ...
        for exp in scope_data['experiment'].unique():
            ...
            ax3.plot(exp_data['step'].values, exp_data['loss'].values, 
    ...
    for bs in sorted(df['batch_size'].unique()):
        ...
        for exp in bs_data['experiment'].unique():
            ...
            ax4.plot(exp_data['step'].values, exp_data['loss'].values, 
...
def plot_zo_analysis(df, output_dir="plots", figsize=(15, 12)):
    ...
    for exp in zo_data['experiment'].unique():
        ...
        if 'grad_norm' in exp_data.columns:
            ax5.plot(exp_data['step'].values, exp_data['grad_norm'].values, 
                    label=exp, alpha=0.7)
...
def plot_fo_vs_zo_comparison(df, output_dir="plots", figsize=(15, 8)):
    ...
    ax1.plot(fo_avg.index.values, fo_avg.values, 'b-', label='FO (Average)', linewidth=2)
    
    # ZO平均曲线
    zo_avg = zo_data.groupby('step')['loss'].mean()
    ax1.plot(zo_avg.index.values, zo_avg.values, 'r-', label='ZO (Average)', linewidth=2)
    ...
    ax4.plot(fo_std.index.values, fo_std.values, 'b-', label='FO', linewidth=2)
    ax4.plot(zo_std.index.values, zo_std.values, 'r-', label='ZO', linewidth=2)
...
def plot_alt_analysis(df, output_dir="plots", figsize=(15, 10)):
    """专门分析ALT模式的结果"""
    if df is None or df.empty:
        return
    
    alt_data = df[df['mode'] == 'ALT'].copy()
    if alt_data.empty:
        print("⚠️  没有ALT实验数据")
        return
    
    fig, axes = plt.subplots(2, 2, figsize=figsize)
    fig.suptitle('ALT Mode Analysis (FO + ZO Alternating)', fontsize=16, fontweight='bold')
    
    # 子图1: 不同alt_zo_steps的loss曲线对比
    ax1 = axes[0, 0]
    if 'alt_zo_steps' in alt_data.columns:
        for alt_zo in sorted(alt_data['alt_zo_steps'].unique()):
            alt_zo_data = alt_data[alt_data['alt_zo_steps'] == alt_zo]
            for exp in alt_zo_data['experiment'].unique():
                exp_data = alt_zo_data[alt_zo_data['experiment'] == exp]
                ax1.plot(exp_data['step'].values, exp_data['loss'].values, 
                        label=f'ALT alt_zo={alt_zo}', alpha=0.7, linewidth=2)
    ax1.set_title('ALT Loss by alt_zo_steps')
    ax1.set_xlabel('Step')
    ax1.set_ylabel('Loss')
    ax1.legend()
    ax1.set_yscale('log')
    ...
    # 子图2: ALT vs ZO vs FO 对比
    ax2 = axes[0, 1]
    # ALT 平均
    if 'alt_zo_steps' in alt_data.columns:
        alt_avg = alt_data.groupby('step')['loss'].mean()
        ax2.plot(alt_avg.index.values, alt_avg.values, 'g-', label='ALT (Average)', linewidth=2)
    
    # ZO
    zo_data = df[df['mode'] == 'ZO'].copy()
    if not zo_data.empty:
        zo_avg = zo_data.groupby('step')['loss'].mean()
        ax2.plot(zo_avg.index.values, zo_avg.values, 'r-', label='ZO (Average)', linewidth=2)
    
    # FO
    fo_data = df[df['mode'] == 'FO'].copy()
    if not fo_data.empty:
        fo_avg = fo_data.groupby('step')['loss'].mean()
        ax2.plot(fo_avg.index.values, fo_avg.values, 'b-', label='FO (Average)', linewidth=2)
    
    ax2.set_title('ALT vs ZO vs FO Comparison')
    ax2.set_xlabel('Step')
    ax2.set_ylabel('Loss')
    ax2.legend()
    ax2.set_yscale('log')
    ...
    # 子图3: 最终loss vs alt_zo_steps
    ax3 = axes[1, 0]
    if 'alt_zo_steps' in alt_data.columns:
        final_losses = alt_data.groupby('alt_zo_steps')['loss'].last().reset_index()
        alt_zo_values = sorted(final_losses['alt_zo_steps'].unique())
        losses = [final_losses[final_losses['alt_zo_steps'] == az]['loss'].iloc[0] 
                 for az in alt_zo_values]
        ax3.plot(alt_zo_values, losses, 'o-', linewidth=2, markersize=8)
        ax3.set_title('Final Loss vs alt_zo_steps')
        ax3.set_xlabel('alt_zo_steps')
    ...
    if 'alt_zo_steps' in alt_data.columns:
        for alt_zo in sorted(alt_data['alt_zo_steps'].unique()):
            alt_zo_data = alt_data[alt_data['alt_zo_steps'] == alt_zo]
            if len(alt_zo_data) > 1:
                initial_loss = alt_zo_data['loss'].iloc[0]
                final_loss = alt_zo_data['loss'].iloc[-1]
                reduction = (initial_loss - final_loss) / initial_loss * 100
                convergence_data.append(reduction)
                labels.append(f'ALT\naz={alt_zo}')
    ...
    if convergence_data:
        ax4.bar(range(len(convergence_data)), convergence_data, alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/alt_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
    print(f"✅ 保存ALT分析图: {output_dir}/alt_analysis.png")
...
def main():
    ...
    parser.add_argument("--alt-analysis", action="store_true", help="生成ALT分析图")
    parser.add_argument("--summary", action="store_true", help="生成总结报告")
    
    args = parser.parse_args()
    
    # 如果没有指定任何选项，默认生成所有图表
    if not any([args.all, args.overview, args.zo_analysis, args.comparison, args.alt_analysis, args.summary]):
        args.all = True
    ...
    if args.all or args.alt_analysis:
        print("\n📊 生成ALT分析图...")
        plot_alt_analysis(df, args.output_dir, tuple(args.figsize))
