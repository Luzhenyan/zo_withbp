...
def train(mode, scope, q, lr, epochs, batch_size, device, plot_file, csv_file=None, log_interval=10, optimizer_type='sgd', alt_zo_steps=100, max_steps=None):
    """主训练函数"""
    
    # 设置
    transformers.logging.set_verbosity_error()
    # 使用本地 whisper 包中的 GPT-2 tokenizer
    ...
    try:
        ...
        print("Falling back to online 'gpt2' tokenizer...")
    ...
    if mode == 'FO':
        ...
        # 共用一个优化器与超参：
        # - SGD: FO 用 SGD 实例；ZO 保持手动 vanilla SGD 更新
        # - Adam/MuDaMW: 同一个优化器实例既用于 FO(.grad)，也用于 ZO(外部 grads)
        if optimizer_type == 'sgd':
            optimizer = torch.optim.SGD(trainable_params, lr=lr)
            print(f"Using optimizer: SGD (FO uses SGD, ZO uses manual vanilla SGD)")
        elif optimizer_type == 'adam':
            optimizer = CustomAdamOptimizer(trainable_params, lr=lr)
            print(f"Using optimizer: Custom Adam (shared for FO & ZO)")
        elif optimizer_type == 'mudamw':
            optimizer = MuDaMWOptimizer(trainable_params, lr=lr)
            print(f"Using optimizer: MuDaMW (shared for FO & ZO)")
        else:
            raise ValueError(f"Unknown optimizer type: {optimizer_type}")
        ...
        for batch in pbar:
            ...
            if mode == 'FO':
                ...
                if step_in_cycle == 0:
                    ...
                    if optimizer_type == 'sgd':
                        # ALT + SGD：ZO 使用手动 vanilla SGD
                        for p, g in zip(trainable_params, grad_paramwise):
                            if g is None:
                                continue
                            p.data -= lr * g
                    else:
                        # ALT + Adam/MuDaMW：共享优化器，传入外部梯度
                        optimizer.step(grads=grad_paramwise)
            ...
            if csv_file and step % log_interval == 0:
                ...
                with open(csv_file, 'a', newline='') as f:
                    ...
                    if mode == 'ZO':
                        ...
                        q_val = q if (step % (1 + alt_zo_steps)) != 0 else 'N/A'
                    else:
                        q_val = 'N/A'
                    writer.writerow([timestamp, epoch+1, step, mode, scope, q_val, lr, batch_size, optimizer_type, loss.item(), grad_norm])
            ...
            # 检查是否达到最大步数（在循环结束时，step 已经增加）
            if max_steps is not None and step >= max_steps:
                print(f"\nReached max_steps={max_steps} at step={step}, stopping training.")
                break
        
        # 如果达到最大步数，退出外层 epoch 循环
        if max_steps is not None and step >= max_steps:
            break
    ...
    plt.plot(losses)
    plt.title(f'Training Loss Curve\nMode={mode}, Scope={scope}, q={q if mode in ["ZO", "ALT"] else "N/A"}, LR={lr}, Optimizer={optimizer_type.upper()}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, required=True, choices=['FO', 'ZO', 'ALT'], help="Optimization mode: First-Order (FO), Zeroth-Order (ZO), or Alternating (ALT).")
    ...
    parser.add_argument("--alt_zo_steps", type=int, default=100, help="In ALT mode: number of ZO steps after each FO step.")
    parser.add_argument("--max_steps", type=int, default=None, help="Maximum number of training steps. If set, training will stop after this many steps.")
    ...
    train(
        ...
        alt_zo_steps=args.alt_zo_steps,
